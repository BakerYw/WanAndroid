...
public class BannerAspectRatioMeasure {
     ...
     * if the parent has specified mode {@code EXACTLY}, and it doesn't exceed measure size if
     * parent
    ...
    public static void updateMeasureSpec(
        ...
        if (shouldAdjust(layoutParams.height)) {
            ...
            int desiredHeight =
                    (int) ((widthSpecSize - widthPadding) / aspectRatio + heightPadding);
            int resolvedHeight = View.resolveSize(desiredHeight, spec.height);
            spec.height = View.MeasureSpec.makeMeasureSpec(resolvedHeight,
                    View.MeasureSpec.EXACTLY);
        } else if (shouldAdjust(layoutParams.width)) {
            int heightSpecSize = View.MeasureSpec.getSize(spec.height);
            int desiredWidth =
                    (int) ((heightSpecSize - heightPadding) * aspectRatio + widthPadding);
