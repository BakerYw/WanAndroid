...
import com.nyw.libwidgets.citypicker.adapter.decoration.GridItemDecoration;
...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    private List<HotCity> mHistoryCity;
    ...
    public CityListAdapter(Context context, List<City> data, List<HotCity> hotData,
                           List<HotCity> historyCity, int state) {
        this.mData = data;
        this.mContext = context;
        this.mHotData = hotData;
        this.mHistoryCity = historyCity;
    ...
    public BaseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_HOT:
                view = LayoutInflater.from(mContext).inflate(R.layout.cp_list_item_hot_layout,
                        parent, false);
            ...
            default:
                view = LayoutInflater.from(mContext).inflate(R.layout.cp_list_item_default_layout
                        , parent, false);
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        ...
        if (holder instanceof LocationViewHolder) {
            ...
            int indexBarWidth =
                    mContext.getResources().getDimensionPixelSize(R.dimen.cp_index_bar_width);
            int itemWidth =
                    (screenWidth - padding - space * (GridListAdapter.SPAN_COUNT - 1) - indexBarWidth) / GridListAdapter.SPAN_COUNT;
        ...
        if (holder instanceof HistoryViewHolder) {
            ...
            final City data = mData.get(pos);
            if (data == null) return;
            GridListAdapter mAdapter = new GridListAdapter(mContext, mHistoryCity);
            mAdapter.setInnerListener(mInnerListener);
            ((HistoryViewHolder) holder).mRecyclerView.setAdapter(mAdapter);
        }
    }
    ...
    public int getItemViewType(int position) {
        if (position == 0 && TextUtils.equals("定", mData.get(position).getSection().substring(0,
                1)))
            return VIEW_TYPE_LOCATION;
        if (position == 1 && TextUtils.equals("历", mData.get(position).getSection().substring(0,
                1)))
            return VIEW_TYPE_HISTORY;
        if (position == 2 && TextUtils.equals("热", mData.get(position).getSection().substring(0,
                1)))
    ...
    public static class HotViewHolder extends BaseViewHolder {
        ...
        HotViewHolder(View itemView) {
            ...
            int space =
                    itemView.getContext().getResources().getDimensionPixelSize(R.dimen.cp_grid_item_space);
