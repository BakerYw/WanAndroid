package com.nyw.libwidgets.scroll.goodsdetail;
...
public class NestedScrollingDetailContainer extends ViewGroup implements NestedScrollingParent2 {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int childWidthMeasureSpec = getChildMeasureSpec(widthMeasureSpec, left + right,
                    params.width);
            int childHeightMeasureSpec = getChildMeasureSpec(heightMeasureSpec, top + bottom,
                    params.height);
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                if (isParentCenter() && mVelocityTracker != null) {
                    ...
                    mCurFlyingType = yVelocity > 0 ? FLYING_FROM_WEBVIEW_TO_PARENT :
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE:
                ...
                boolean isInNestedChildViewArea = isTouchNestedInnerView((int) ev.getRawX(),
                        (int) ev.getRawY());
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            switch (mCurFlyingType) {
                ...
                case FLYING_FROM_PARENT_TO_WEBVIEW://父控件向WebView滑动
                    ...
                    break;
                case FLYING_FROM_RVLIST_TO_PARENT://RecyclerView向父控件滑动，fling
                    // 事件，单纯用于计算速度。RecyclerView的flying事件传递最终会转换成Scroll事件处理.
                    if (getScrollY() != 0) {
                        invalidate();
    ...
    private void parentFling(float velocityY) {
        mScroller.fling(0, getScrollY(), 0, (int) velocityY, 0, 0, Integer.MIN_VALUE,
                Integer.MAX_VALUE);
        invalidate();
    }
